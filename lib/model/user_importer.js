...
function add_user(args) {
  var validate_args = Joi.validate(args, add_user_interface_schema);
...
function add_users_in_bulk(args) {
  var bulk_header = args.bulk_header,
  ...
  .then(() => {
    return Promise.map(bulk_data, vector => {
      var email = vector[ email_vector_index ];
...
function validate_email_to_be_free(args) {
  var validate_args = Joi.validate(args, validate_email_to_be_free_schema);
