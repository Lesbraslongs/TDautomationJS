...
router.post('/import-sample/', function(req, res){
  ...
  req.user
    ...
    .then(company => {
      ...
      var content = company.users.map( user => [
        user.email,
        user.name,
        user.lastname,
...
router.post('/edit/:user_id/', function(req, res){
  ...
  .then(function(){
    ...
    var adjustment = new_user_attributes.adjustment;
...
function promise_user_list_data_for_rendering(args) {
  ...
  var users_info_for_rendering = users_info.map(ui => ({
...
function users_list_as_csv(args) {
  var users_info = args.users_info,
  ...
  var content = [['email', 'lastname', 'name', 'department', 'remaining allowance', 'days used']];
